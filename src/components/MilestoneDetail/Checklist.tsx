...
function ChecklistItem({ item, color, isDark, index, onToggle, onFoodTrialOutcome }: ChecklistItemProps) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          borderColor: item.completed ? (isDark ? '#4A8B7F' : '#B5C4B1') : color,
          backgroundColor: item.completed ? (isDark ? '#4A8B7F' : '#B5C4B1') : 'transparent',
        }}
      >
        {item.completed && (
